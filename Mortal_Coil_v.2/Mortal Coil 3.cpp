	...
	while (1) {
		...
		for (int j = 0; exit && j < w; j++) {
			for (int i = 0; exit && i < h; i++) {
				...
				if (exit) printf("(%2d, %2d) passed (%.3lf seconds)\n", j, i, (clock() - part_s) / 1000.0);
			}
		}
...
int dfs(int t[][WIDTH], int a, int b)
{
	...
	while (k) {
		...
		int check = 0;
		int exp_val = explore(t, x, y); //길의 끝이 2개 있는지 검사

		if (exp_val) { //가능
			int bfs_val = bfs(t); //타일이 이어져 있는지 검사
